...
We apply adjustment curves by distributing adjustment keyframes along the trajectory. Each keyframe is associated with a rotation and per frame values are interpolated along the curve to ensure smooth changes. The adjustment curve is applied by accumulating rotations along the entire animation, so that an adjustment rotation at frame $n$ influences all frames $>n$. An example in shown in Fig. \ref{fig:method:adjustment}. 

Adjustment curves can be used as a fitting procedure with $argmin$

In this work we only perform drift adjustments on straight animation segments, ie. coming into a turn and coming out of a turn, while the turn itself is left unchanged. By minimizing the adjustment knot value against the error between the adjusted trajectory segment and a least squares linear fit. 

We do two types: Make lines straighter by optimizing against error of linear least sqaures fit line. And by optimizin against error againdt other trajecotry
\begin{figure}
    \centering
