...
We apply adjustment curves by distributing $\parame,$ adjustment keyframes along the trajectory. With high $\counte$-values adjustment values change frequently. By decreasing $\counte$ we achieve smoother adjustment. Each keyframe is associated with a rotation and per frame values are interpolated along the curve to ensure smooth changes. The adjustment curve is applied by accumulating rotations along the entire animation, so that an adjustment rotation at frame $n$ influences all frames $>n$. An example in shown in Fig. \ref{fig:method:adjustment}. 

Adjustment curves can align evaluation space vectors $v_0,v_1\in R^{3t}$ by an optimization
\begin{subequations}
\begin{align}
    \arg\min_{\parame}{
...
\end{align}
\end{subequations}
In the Movement Model fitting (\ref{eq:optim:single}) $\counte$ is set so adjustments keyframes are distributed at 2Hz. 

For motion alignment we optimize to fit $v \in R^{3t}$ against a linear least squares fit.
\begin{subequations}
\begin{align}
    \arg\min_{\parame}{
...
\end{align}
\end{subequations}
The procedure only works for segments of the animation that are almost straight. We use the heuristic to do local alignments guided by Control Genome changes. Each change triggers a new local alignment, and $\omega$ is a weight vector used to disable alignment close to the local end points. In this case the keyframes are distributed only at 1Hz. 
\begin{figure}
    \centering
...
\magnus{Section should end by defining W}
...
\begin{itemize}
    \item {\bf Aligment}
    \item {\bf Movement model fitting}
    \item {\bf Post Alignment}
\end{itemize}


We use autodiff features of pytorch.

Primitives continuous


Pre alignment. Ignore turns. Straight segment are applied a least squares linear regression.

Control genome estimation 
    Estimate rought direction change points in animation (Ie where we see change) Use tangents. First order diffs and fitted linear thing, when avaialb.e

Movement Model fit.
    L2 regularization of parameters, but only if they have gradient. Data might not cover the entire parameter space. Dont want optimization on onused parameters.. General construct is parameter-scale * parameter. Scale is constant.
    Initialize model to first control genome.


    Control Genommes direction and times and can be adjusted.
        Tanh activation to control change in genome direction


Post alignments. 
    Adjustment curve at fixed interval. Destructive to animation. Gives meassure of quality of fit. It is difficult to quantify the amount of unrealisme introduced due to the procedure, so we instead refer to the accompyining video as a supplement to the plots. 
...
We have knots in normalized range. Multiply with constant. Makes regularization and learning rate work
'