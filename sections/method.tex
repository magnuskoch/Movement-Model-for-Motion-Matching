...
\begin{equation}
    ...
    \end{cases}\,.
\end{equation}
To describe changes in $\blend$ over time $\dt$ each $\interpolator \in \interpolators$ defines a mapping from $\dt$ to a change in interpolation progression $\dipweight$, where  $0=>\ipweight{<=1}$. 
\begin{subequations}
\begin{align}
\dipweight &\leftarrow \interpolator_{a,b}(\dt)\\
\weight_{a,t+\dt},\weight_{b,t+\dt} &\leftarrow\interpolator_{a,b}(\weight_{a,t},\weight_{b,t}, \dipweight)
\end{align}
\end{subequations}
...
where $\mode_g$ and $\kine_g$ is the desired locomotion mode and kinematics respectively. 
...
\end{align}
\end{subequations}
Where $\dt$ is a time step. Intuitively $\sfunc_{\blend}$ moves $\blend$ towards $w_{l_{g}}=1$ and $w_{i\neq{l_g}}=0$ using the interpolations rules in $\interpolators$. To do so, we first identify each interpolator that describes a transition into $\modes_g$, and them computre a weighted sum of $\Delta{\weight}$
