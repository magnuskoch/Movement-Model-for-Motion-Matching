...
\begin{subequations}
\begin{align}
    [\pos,\facing,\controlface,\controlmove]_t,\dt&\rightarrow\dtpos_t,\dtfacing_t\label{eq:move:update}\\
    \pos_{t+\dt}=\pos + \dt\dtpos\\
    \facing_{t+\dt}=\facing + \dt\dtfacing
\end{align}
\end{subequations}
Thus, the Movement Model supplies motion planning by the gradual state transition, with the conflicting goals of quick correspondence to the control parameters and adherence to the characteristics of the source animation.
...
\begin{subequations}
\begin{align}
    \modelspeed&\leftarrow{}\spring(\statespeed,\intermediaryspeed,\intermediary_0)\\
    \intermediaryspeed&\leftarrow{}\mapt([\stateangularspeed,\diffmove],\param_0)\\
    \intermediary_0&\leftarrow{}\mapo(\statespeed-\intermediaryspeed, \param_1)
\end{align}
\end{subequations}
Here $\param_0$ models the acceleration of the model depending on the relative change in speed, and $\param_1$ models planning of the target speed depending on the current angular speed and the required change to movement direction.

Modeling of changes to movement and facing directions are added.
\begin{subequations}
\begin{align}
    \modelmove&\leftarrow\spring(\statemove,\controlmove,{spring}_m)\\ 
    \modelfacing&\leftarrow\spring(\statefacing,\controlface,{spring}_f)\\
    {spring}_m&\leftarrow{}\mapt([\statespeed,\diffmove], \param_2)\\
    {spring}_f&\leftarrow{}\mapt([\statespeed,\difffacing], \param_3)
\end{align}
\end{subequations}
