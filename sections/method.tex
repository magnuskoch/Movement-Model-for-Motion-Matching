...
\begin{figure}
    ...
    \caption{Control Genome.Please describe in caption what reader should learn from the figure. }
...
As an example we propose a Movement Model for plane locomotion typical for 3rd person computer games  \kenny{how can you prove it is typical? any cites for this?} that strikes a compromise between simplicity and expressiveness. We use the convention that variabel annotated with a bar such as $\intermediary\speed$ refers to intermediary values used temporarily during the model update. 
%
