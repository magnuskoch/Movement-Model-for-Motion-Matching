...
\begin{equation}
\dweight_{a} \leftarrow \efunc_{\interpolators}\left( \interpolator^{a \rightarrow b}, \weight_{b}, \dt\right)
\end{equation}
...
\begin{equation}
\theta_{k}=\dfrac{\weight_k}{\sum_{\interpolator_j^{a\rightarrow g} \in\interpolators}\weight_j},
\quad \forall a \neq g \,.
\end{equation}
...
\end{equation}
Finally each locomotion mode map to changes in kinematics with.
\begin{equation}
\delspeed_j, \delmove_{,j}, \delfacing_{,j} \leftarrow \efunc_{\modes}\left( \mode_j, \kine_t, \kine_g, \dt \right)
\end{equation}
Now \ref{eq:step:modes} is evaluated by
\begin{equation}
\sfunc_{\kine,t} \leftarrow \sum_{\mode_k \in \modes} \weight_{k,t+1} \efunc_{\modes}
\end{equation}
And the full kinematics state is populated by
\begin{subequations}
\begin{align}
   \move_{t+1} &= \delmove \move_t \\
   \speed_{t+1} &= \speed_t + \delspeed \\
   \pos_{t+1} &= \pos_t + \move_{t+1} \uv \speed \dt \\
   \facing_{t+1} &= \delfacing \facing_t \\
   \dtmove &= \move_{t+1}^{-1}\move_t \\
   \dtfacing &= \facing_{t+1}^{-1}\facing_t
\end{align}
\end{subequations}
